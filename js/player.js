/* ═══════════════════════════════════════════
   NovaCiné — player.js  (Video.js v7 — Final)
   Auto-hide : CSS :hover + classe js-playing
   Fullscreen : natif navigateur sur player-wrap (desktop)
               + vidéo native sur iOS
   ═══════════════════════════════════════════ */
...
/* ══════════════════════════════
   CSS
   Auto-hide = CSS pur :
   - En pause → barre toujours visible
   - En lecture → barre visible au hover, cachée sinon
   - En fullscreen (classe nc-fs sur player-wrap) → même logique
══════════════════════════════ */
function injectTheme() {
  ...
  s.textContent = `
    ...
    /* Neutraliser TOUT l'auto-hide VJS — on override avec !important */
    ...
    #nc-wrap .vjs-play-progress::before { color: #f0c060 !important; font-size: 13px !important; top: -0.3em !important; }
    ...
    #nc-wrap .vjs-menu-item { font-size: .8rem !important; color: rgba(220,232,247,.8) !important; padding: .5rem 1rem !important; }
    ...
    /* ── VJS fullscreen control caché (on utilise le nôtre) ── */
    #nc-wrap .vjs-fullscreen-control { display: none !important; }
    ...
    #nc-wrap .video-js:hover .vjs-big-play-button {
      background: rgba(232,160,32,.18) !important; border-color: rgba(232,160,32,.9) !important;
    ...
    #player-wrap:fullscreen #nc-wrap  { width:100%!important; height:100%!important; aspect-ratio:unset!important; }
    /* Fond modal noir (pas gris) */
    ...
    @media (max-width: 640px) {
      ...
      /* Sur mobile : barre toujours visible (pas d'auto-hide) */
      #player-wrap.nc-playing #nc-wrap .vjs-control-bar { opacity: 1 !important; }
      #player-wrap.nc-playing { cursor: default; }
    }

    /* ── Loading / Fallback / iOS ── */
...
/* ══════════════════════════════
   CLASSE nc-playing sur player-wrap
   CSS fait tout le reste
══════════════════════════════ */
function setPlaying(playing) {
  var pw = document.getElementById('player-wrap');
  if (!pw) return;
  if (playing) pw.classList.add('nc-playing');
  else         pw.classList.remove('nc-playing');
}
...
/* ══════════════════════════════
   FULLSCREEN
   Desktop : requestFullscreen sur player-wrap
   iOS     : webkitEnterFullscreen sur la vidéo
══════════════════════════════ */
...
function playerToggleFS(){
  if(IS_IOS){
    /* iOS : fullscreen natif sur l'élément vidéo */
    var vid=document.getElementById('nc-vjs-el');
